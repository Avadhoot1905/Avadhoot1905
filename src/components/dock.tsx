...
export function Dock({ apps, onAppClick }: DockProps) {
  ...
  // Filter apps for mobile: only social media links
  ...
  const showSocialSeparator = Boolean(socialApps.length > 0 && (finderApp || regularApps.length > 0))

  const renderApp = (app: DockProps['apps'][number]) => (
    <motion.div
      key={app.id}
